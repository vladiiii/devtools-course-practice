...
int quantity_2, double price_1, double price_2) {
    double dq = (double)(quantity_2 - quantity_1);
    double dp = price_2 - price_1;

    if ((fabs(dq) / (double)quantity_1 < 0.05) && 
    fabs(dp) / price_1 < 0.05) {
        return (dq / (double)quantity_1) / (dp / price_1);
    } else {
        double p_ = (price_1 + price_2) / 2.0;
        double q_ = (double)(quantity_1 + quantity_2) / 2.0;
...
int quantity_2, double income_1, double income_2) {
    ...
    return (dq / (double)quantity_1) / (di / income_1);
}
...
int quantity_2_product_1, double price_1_product_2, double price_2_product_2) {
    double dq = (double)(quantity_2_product_1 - quantity_1_product_1);
    double dp = price_2_product_2 - price_1_product_2;

    return (dq / (double)quantity_1_product_1) / (dp / price_1_product_2);
}